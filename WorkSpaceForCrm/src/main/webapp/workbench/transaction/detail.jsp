...
<%@ page import="com.bjpowernode.setting.bean.DicValue" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.bjpowernode.workbench.bean.Tran" %>
...
<script type="text/javascript">
	...
	* i:需要改变的阶段对应的下标*/
	...
	function changeIcon(stage,index1){
		...
		//如果当前阶段可能性为0,前七个一定是黑圈，后两个一个红叉，一个黑叉
		...
		//如果当前阶段可能性不为0，前七个绿圈，绿色标记，黑圈，后两个黑叉
		}else {
			//遍历前七个，绿圈，绿色标记，黑圈
			for (var i = 0; i<point; i++){
				//如果是当前阶段
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 250px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/tran/edit.do?id=${t.id}';"><span class="glyphicon glyphicon-edit"></span> 编辑</button>
			<button type="button" class="btn btn-danger" id="tranDeleteBtn"><span class="glyphicon glyphicon-minus"></span> 删除</button>
		</div>
	</div>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//判断当前阶段
			//如果当前阶段可能性为0，前7个一定是黑圈，后两个一个红叉，一个黑叉
			if ("0".equals(currentPossibility)){
				//取得每一个遍历出的阶段，根据每一个遍历出来的阶段取其可能性
				for (int i = 0; i< dvList.size(); i++){
					DicValue dv = dvList.get(i);
					String listStage = dv.getValue();
					String listPossibility = pMap.get(listStage);

					//如果遍历出的阶段可能性为0，说明是后两个，一个是红叉，一个是黑叉
		...
		<%
					...
					//如果遍历出来的阶段可能性不为0，说明是前7个，并且一定是黑圈
					}else {
		%>
		<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')"
